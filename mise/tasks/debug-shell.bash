...
if [[ ${usage_bundle:-} == 'true' ]]; then
	shell="$(nix build --no-link --print-out-paths --file . outputsForCurrentSystem.packages.shell-bundle)"
else
	shell_directory="$(nix build --print-out-paths --no-link --file . outputsForCurrentSystem.packages.shell)/bin"
