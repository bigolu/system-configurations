...
# comma
#
# Wrap `comma`, and not `,`, since completions are only defined for `comma`.
function , --wraps comma
    # `--with-nth` removes the '.out' extension from the entries.
    FZF_DEFAULT_OPTS="$FZF_DEFAULT_OPTS --separator '' --height 10 --margin 0,2,0,2 --preview-window right,75%,border-left --preview 'nix-search --details --max-results 1 --name (string sub --end -4 {})' --delimiter '.out' --with-nth '{1}'" COMMA_PICKER=fzf command , --cache-level 0 $argv
end
...
abbr --add --global t broot --cmd "':toggle_preview;:toggle_watch'"
...
function nix-py
    nix shell --impure --expr "(import <nixpkgs> {}).python3.withPackages (p: with p; [$argv])"
end
