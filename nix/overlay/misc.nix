...
in
{
  ...
  nix-shell-interpreter = final.writeShellApplication {
    ...
    text = ''
      ...
      # set by nix-shell so when I execute the shell again, the temporary
      # directory will not exist which will break any programs that try to access
      # it. To get around this, I use this script as my shebang interpreter and
      # then I unset the variables. I'm thinking that once nix development
      # environments are no longer made from build-debugging environments, this
      # won't be an issue anymore[1]. Otherwise, I should see if cached-nix-shell
      # could allow users to specify variables that shouldn't get cached.
      #
